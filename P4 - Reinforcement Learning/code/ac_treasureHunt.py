...
def dumpCSV(nIter, times,rewards,steps,convergence,discount , method, world ,qval= None):
    ...
    if qval :
        fname = 'TH_logs/QL' + '/{} {} D-{}.csv'.format(world,method,discount)
        ...
        fname = 'TH_logs/' + method + '/{} {} D-{}.csv'.format(world,method,discount)
...
if __name__ == '__main__':
    world = 'tHunt'
    NUM_INTERVALS   = MAX_ITERATIONS = 100 ;  ## 200
    evalTrials      = 50;
    ...
    print ' ====== > Calling ac_thunt_2 :  Max X, Y - ' , maxX , ' ' , maxY
...
#                for epsilon in [0.2, 0.6, 0.9]:
                    for disc in discount:
                        ...
                        print "//Treasure Hunt {} Iteration Analysis//".format(Qname)
                        print Qname    
...
#                    for disc in discount:
                        ...
                        Qname = 'QL L{:0.1f} q{:0.1f} E{:0.1f}'.format(lr,qInit,epsilon)
                        agent = QLearning(domain, disc ,hashingFactory, qInit, lr, epsilon,900) #300
                        ...
                        for nIter in iterations: 
                            ...
                            startTime = clock()    
                            ea = agent.runLearningEpisode(env,900)   #300
                            if len(timing[Qname])> 0:
                                timing[Qname].append(timing[Qname][-1]+clock()-startTime)   
                            else:
                                timing[Qname].append(clock()-startTime)             
